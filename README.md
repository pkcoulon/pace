...
```sh
make app      # compile et assemble build/UsageBar.app
make run      # (re)lance l'app
```

`make app` produit `build/UsageBar.app`, signé ad hoc. Copie-le dans
`/Applications` si tu veux le garder. Active « Lancer au démarrage » dans les
réglages de l'app pour qu'il démarre avec la session.
...
- **Popover** (clic) : une carte par provider, barres 5 h et hebdo, compte à
  ...
  usage / crédits s'ils existent. En bas : dernier refresh, Rafraîchir, Réglages,
  Quitter.
- **Réglages** : activer/désactiver chaque provider, afficher « 5 h seul » ou
  « 5 h + hebdo », intervalle de refresh (1 / 3 / 5 min), seuils de notification,
  lancer au démarrage, et le champ session key Claude avec bouton Tester.
- **Notifications** : une alerte native à 80 % et à 95 % sur chaque fenêtre,
...
UsageBar ne rafraîchit jamais ce token lui-même : si l'appel renvoie 401, le
...
UsageBar ne contacte que les hôtes strictement nécessaires, aucune télémétrie,
...
```sh
# nécessite les droits admin ; filtre le processus UsageBar
sudo lsof -i -nP | grep -i usagebar
